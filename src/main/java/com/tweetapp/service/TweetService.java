...
public class TweetService {
	...
	/*
	 * @Autowired private KafkaProducerConfig producer;
	 */
	...
	public ResponseEntity<Envelope<String>> postTweet(String userName, TweetRequest tweetRequest) {
		...
		Optional<User> findByEmailIdName = userRepository.findByFirstName(userName);
	...
	public ResponseEntity<Envelope<String>> updateTweet(String userName, int tweetId, TweetRequest tweetRequest) {
		...
		tweet.setTweet(TweetConstant.TWEET);
	...
	public ResponseEntity<Envelope<String>> likeTweet(String userName, int tweetId) {
		 ...
		 * tweetAndUserValidation(userName, tweetId); Optional<Tweet> findById =
		 * tweetRepository.findById(tweetId); Tweet tweet = findById.get(); Map<String,
		 * Integer> OldlikesMap = tweet.getLikes(); Map<String, Integer> updatedLikesMap
		 * = new HashMap<>(); if (OldlikesMap != null)
		 * updatedLikesMap.putAll(OldlikesMap); updatedLikesMap.put(userName, 1);
		 * tweet.setLikes(updatedLikesMap); Query query = new Query();
		 ...
		 * //log.info(TweetConstant.EXITING_RESPONSE_LOG, "likeTweet",
		 * tweet.getLikes()); tweet = mongoOperations.findAndModify(query, update,
		 * Tweet.class); if (tweet == null) throw new
		 * TweetAppException(HttpStatus.INTERNAL_SERVER_ERROR.value(),
		 * HttpStatus.INTERNAL_SERVER_ERROR, "Error While Liking");
		 */
